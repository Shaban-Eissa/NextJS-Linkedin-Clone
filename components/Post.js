    ...
    >
      <div className="flex items-center px-2.5 cursor-pointer">
        ...
        <div className="mr-auto ml-2 leading-none">
          <h6 className="font-medium">{post.username}</h6>
          <p className="text-sm dark:text-white/75">{post.email}</p>
          <TimeAgo
        ...
        {modalPost ? (
          ...
          <IconButton>
            <MoreHorizRoundedIcon className="dark:text-white/75 h-5 w-5" />
          </IconButton>
        )}
      </div>
      {post.input && (
      ...
      <div className="flex justify-evenly items-center dark:border-t border-gray-600/80 mx-2.5 pt-2 text-black/60 dark:text-white/75">
        {modalPost ? (
          ...
          >
            {liked ? <ThumbUpRoundedIcon /> : <ThumbUpRoundedIcon />}
